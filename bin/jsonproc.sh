...
function crecsv2split {
  ...
  # buy filter
  mlr -c --from "$1" filter '$cardNumber == "428942******5024"' then cut -f booking,ownerAccountNumber,amount,partner,partnerName,reference,cardNumber then sort -f booking > "$3"
  # Int filter
  mlr -c --from "$1" filter 'is_empty($cardNumber) && is_empty($partnerNumber )' then cut -f booking,ownerAccountNumber,amount,partner,partnerName,reference,cardNumber then sort -f booking > "$4"
}
